...
#ifndef _RGB_LED_TABLE_H_
#define _RGB_LED_TABLE_H_

#include "board.h"
#include "rgb_led_action.h"

#define LED_ACTION_MAX              get_rgb_para_len()
#define LED_ACTION_DEFAULT_MAX      11
...
typedef struct _kr_s {
    uint8_t b;
    uint8_t g;
    uint8_t r;
    uint8_t m;
} __attribute__ ((aligned (4))) key_led_color_t;


typedef struct
{
    uint32_t itf;
    key_led_color_t table[BOARD_LED_COLOR_LAYER_MAX][BOARD_KEY_LED_NUM];
}__attribute__ ((aligned (4))) key_led_color_info_t;
