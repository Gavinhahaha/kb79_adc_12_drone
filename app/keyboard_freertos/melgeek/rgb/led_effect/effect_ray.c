...
static void do_set_ray(uint8_t oki, int ri, int ci, uint16_t dly, uint8_t r, uint8_t g, uint8_t b)
{
    if ((ri >= 0 && ri < BOARD_LED_ROW_NUM) && (ci >= 0 && ci < BOARD_LED_COLUMN_NUM))
    {
        ...
        if (gmx.pla[gbinfo.lm.cur_lmm].at != LED_ACTION_RAY)
        {
            return;
        }
        ...
        if (pkr->m == LED_ACTION_RAY || pkr->m == 0)
        {
            pkts->ltkrr = r;
            pkts->ltkrg = g;
            pkts->ltkrb = b;
...
static void do_ray(uint8_t ki, uint32_t count, uint8_t cr, uint8_t cg, uint8_t cb)
{
    uint32_t dly = 0;
    uint8_t r = 0, c = 0;
    ...
    for (int i = 0;; ++i)
    {
        ...
        if ((ci0 - 4) < BOARD_LED_COLUMN_NUM)
        {
            flag = true;
            for (int k = 0; k <= i; ++k)
            {
                do_set_ray(ki, r - i + k, c + k, dly, cr, cg, cb);
            }
        }
...
void keyp_ray(uint8_t ki, led_action_t *pla)
{
    ...
    if (pkts->kps == KEY_STATUS_U2D)
    {
        ...
        do_ray(ki, dly, pkts->ltkrr, pkts->ltkrg, pkts->ltkrb);
    }
}