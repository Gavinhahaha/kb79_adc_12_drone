...
void Mg_SEGGER_RTT_printf(char* tag,const char* fmt, ...)
{
      static char _buffer[BUFFER_SIZE_UP+16]; 
      int _offset = 0; 
      /* 安全地写入前缀 */ 
      _offset = snprintf(_buffer, sizeof(_buffer), "[%s] ", tag); 
      if ( (_offset > 0)  && (_offset < (int)sizeof(_buffer))) { 
          va_list _ap; 
          va_start(_ap, fmt); 
          /* 使用vsnprintf和va_list */ 
          vsnprintf(_buffer + _offset, sizeof(_buffer) - _offset, fmt, _ap); 
          va_end(_ap); 
          SEGGER_RTT_WriteString(0, _buffer); 
      } 
  }
/*************************** End of file ****************************/
