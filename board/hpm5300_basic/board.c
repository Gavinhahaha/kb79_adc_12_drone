 ...
 */
...
#ifdef BOARD_TEST_GPIO_CTRL

 void board_startup_output(void)
 {
     HPM_IOC->PAD[IOC_PAD_PA09].FUNC_CTL = IOC_PA09_FUNC_CTL_GPIO_A_09;
     gpio_set_pin_output(BOARD_TEST_GPIO_CTRL, BOARD_TEST_GPIO_INDEX, BOARD_TEST_GPIO_PIN1);
     gpio_write_pin(BOARD_TEST_GPIO_CTRL, BOARD_TEST_GPIO_INDEX, BOARD_TEST_GPIO_PIN1, 0);

     for (uint32_t i = 0; i < 2; i++)
     {
         gpio_toggle_pin(BOARD_TEST_GPIO_CTRL, BOARD_TEST_GPIO_INDEX, BOARD_TEST_GPIO_PIN1);

         board_delay_ms(50);
         gpio_toggle_pin(BOARD_TEST_GPIO_CTRL, BOARD_TEST_GPIO_INDEX, BOARD_TEST_GPIO_PIN1);

         board_delay_ms(50);
         DBG_PRINTF("toggling led %u/%u times\n", i + 1, 2);
     }
 }
#endif
 ...
 {
     DBG_PRINTF("==============================\n");
     DBG_PRINTF(" %s clock summary\n", BOARD_NAME);
     DBG_PRINTF("==============================\n");
     DBG_PRINTF("cpu0:\t\t %luHz\n", clock_get_frequency(clock_cpu0));
     DBG_PRINTF("ahb:\t\t %luHz\n", clock_get_frequency(clock_ahb));
     DBG_PRINTF("mchtmr0:\t %luHz\n", clock_get_frequency(clock_mchtmr0));
     DBG_PRINTF("xpi0:\t\t %luHz\n", clock_get_frequency(clock_xpi0));
     DBG_PRINTF("==============================\n");
 }
 ...
 void board_timer_create(uint32_t ms, board_timer_cb cb)
 {
     ...
     config.reload = gptmr_freq / 1000 / 1000 * ms;
 ...
 {
     gpio_toggle_pin(BOARD_LED_GPIO_CTRL, BOARD_TEST_GPIO_INDEX, BOARD_TEST_GPIO_PIN);
 }
 ...
 void board_init_spi_pins_with_gpio_as_cs(SPI_Type *ptr)
 {
    ...
    if (ptr == HPM_SPI1) 
    {
        gpio_set_pin_output_with_initial(BOARD_SPI_CS_GPIO_CTRL, GPIO_GET_PORT_INDEX(BOARD_SPI_CS1_PIN), GPIO_GET_PIN_INDEX(BOARD_SPI_CS1_PIN), !BOARD_SPI_CS_ACTIVE_LEVEL);
        gpio_set_pin_output_with_initial(BOARD_SPI_CS_GPIO_CTRL, GPIO_GET_PORT_INDEX(BOARD_SPI_CS2_PIN), GPIO_GET_PIN_INDEX(BOARD_SPI_CS2_PIN), !BOARD_SPI_CS_ACTIVE_LEVEL);
        gpio_set_pin_output_with_initial(BOARD_SPI_CS_GPIO_CTRL, GPIO_GET_PORT_INDEX(BOARD_SPI_CS4_PIN), GPIO_GET_PIN_INDEX(BOARD_SPI_CS4_PIN), !BOARD_SPI_CS_ACTIVE_LEVEL);
    }
    else if (ptr == HPM_SPI2) 
    {
        gpio_set_pin_output_with_initial(BOARD_SPI_CS_GPIO_CTRL, GPIO_GET_PORT_INDEX(BOARD_SPI_CS0_PIN), GPIO_GET_PIN_INDEX(BOARD_SPI_CS0_PIN), !BOARD_SPI_CS_ACTIVE_LEVEL);
        gpio_set_pin_output_with_initial(BOARD_SPI_CS_GPIO_CTRL, GPIO_GET_PORT_INDEX(BOARD_SPI_CS3_PIN), GPIO_GET_PIN_INDEX(BOARD_SPI_CS3_PIN), !BOARD_SPI_CS_ACTIVE_LEVEL);
    }
 }
 ...
 uint32_t board_init_uart_clock(UART_Type *ptr)
 {
     ...
     if (ptr == HPM_UART0) {
         ...
         clock_set_source_divider(clock_uart4, clk_src_pll0_clk2, 8);
         clock_add_to_group(clock_uart4, 0);
 ...
 void board_init_i2c(I2C_Type *ptr)
 {
     ...
     if (stat != status_success) {
         DBG_PRINTF("failed to initialize i2c 0x%lx\n", (uint32_t) ptr);
         while (1) {
         }
     }
